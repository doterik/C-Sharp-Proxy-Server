...
namespace Brotli
{
	public static class BrotliCompression
	{
		...
		public static void Compress(Stream input, Stream output) => Compress(CreateDefaultParameters(), input, output);
		...
		public static void Compress(CompressionParameters parameters, Stream input, Stream output)
		{
			...
			if (ok == 0) throw new BrotliException();
		}
		...
		private class SequentialStream : ISequentialStream
		{
			...
			public SequentialStream(Stream stream) => _stream = stream;
		...
		public static void Decompress(Stream input, Stream output)
		{
			...
			try
			{
				...
				if (offsetOut != IntPtr.Zero) output.Write(outputBuffer, 0, offsetOut.ToInt32());
			}
			finally
			{
				DestroyState(state);
		...
		private static IntPtr CreateState()
		{
			...
			return hr == 0 ? state : throw new Win32Exception(hr);
		}
		...
		{
			return IntPtr.Size == 4
				? CompressStream86(ref parameters, input, output)
				: CompressStream64(ref parameters, input, output);
		}
		...
		[DllImport("winbrotli.x86.dll", EntryPoint = "CreateState")] private static extern int CreateState86(out IntPtr state);
		[DllImport("winbrotli.x86.dll", EntryPoint = "DestroyState")] private static extern int DestroyState86(IntPtr state);
		[DllImport("winbrotli.x64.dll", EntryPoint = "CreateState")] private static extern int CreateState64(out IntPtr state);
		[DllImport("winbrotli.x64.dll", EntryPoint = "DestroyState")] private static extern int DestroyState64(IntPtr state);
	...
	public struct CompressionParameters
	{
		...
		[MarshalAs(UnmanagedType.I1)] public bool EnableDictionary;
		[MarshalAs(UnmanagedType.I1)] public bool EnableTransforms;
		[MarshalAs(UnmanagedType.I1)] public bool GreedyBlockSplit;
		[MarshalAs(UnmanagedType.I1)] public bool EnableContextModeling;
	}

	public enum CompressionMode
	{
		Generic = 0, // Default compression mode. The compressor does not know anything in advance about the properties of the input.
		Text = 1,    // Compression mode for UTF-8 format text input.
		Font = 2     // Compression mode used in WOFF 2.0.
	}

	[Serializable]
	public class BrotliException : IOException
	{
		public BrotliException() : base(string.Empty) { }
		...
		public BrotliException(Exception innerException) : base(null, innerException) { }
		protected BrotliException(SerializationInfo info, StreamingContext context) : base(info, context) { }
	}
}
